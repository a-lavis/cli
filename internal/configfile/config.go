// Package configfile handles loading and writing bk CLI configuration files.
package configfile
...
type fileConfig struct {
	...
	AllowKeyringInCI bool                 `yaml:"allow_keyring_in_ci,omitempty"`
}
...
// LegacyTokenForOrg gets the legacy API token persisted in user/local config files.
//
// This is read-only compatibility behavior for pre-keyring token storage.
func (conf *Config) LegacyTokenForOrg(org string) string {
	return firstNonEmpty(
...
// HasLegacyTokenForOrg reports whether a legacy API token exists for org in
// user/local config files.
func (conf *Config) HasLegacyTokenForOrg(org string) bool {
	if org == "" {
		return false
	}
	return conf.LegacyTokenForOrg(org) != ""
}

// AllowKeyringInCI returns whether keyring usage is allowed when CI is set.
// This setting is user-config only and intentionally ignores local config.
func (conf *Config) AllowKeyringInCI() bool {
	return conf.user.AllowKeyringInCI
}

// SetAllowKeyringInCI sets whether keyring usage is allowed when CI is set.
// This setting is user-config only.
func (conf *Config) SetAllowKeyringInCI(v bool) error {
	conf.user.AllowKeyringInCI = v
	return conf.writeUser()
}
